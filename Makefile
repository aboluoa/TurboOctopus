...
turbooctopus:
	$(GORUN) build/ci.go install ./cmd/turbooctopus
	@echo "Done building."
	@echo "Run \"$(GOBIN)/turbooctopus\" to launch TurboOctopus."

receiver:
	$(GORUN) build/ci.go install ./cmd/receiver
	@echo "Done building."
	@echo "Run \"$(GOBIN)/receiver\" to launch receiver."
...
geth-linux:
	env GO111MODULE=on GOARCH=amd64 GOOS=linux go build -ldflags="$(ldflags)" -o build/bin/geth-linux ./cmd/geth
