...
var (
	peersRDLock   sync.Mutex
	dialingRDLock sync.Mutex
	historyRDLock sync.Mutex
)
...
type dialSetupFunc func(net.Conn, connFlag, *enode.Node, int) error
	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				...
				d.startDial(newDialTask(node, dynDialedConn, 0))
			}
		...
		case node := <-d.addStaticCh:
			...
			task := newDialTask(node, staticDialedConn, 0)
			d.static[id] = task
...
func newDialTask(dest *enode.Node, flags connFlag, dataId int) *dialTask {
	return &dialTask{dest: dest, flags: flags, staticPoolIndex: -1, dataId: dataId}
}
...
func (t *dialTask) dial(d *dialScheduler, dest *enode.Node) error {
	...
	if err != nil {
		...
		node_logs.Error(fmt.Sprintf("id:%v %v", dest.ID().String(), cleanupDialErr(err)))
		return &dialError{err}
	}
	mfd := newMeteredConn(fd, false, &net.TCPAddr{IP: dest.IP(), Port: dest.TCP()})
	return d.setupFunc(mfd, t.flags, dest, t.dataId)
}
