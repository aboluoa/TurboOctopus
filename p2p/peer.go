...
loop:
	for {
		...
		case err = <-writeErr:
			...
			if err != nil {
				...
				node_logs.Error("end peer run from writeErr 111", "peer", p.ID(), "err:", err)
				break loop
			}
			writeStart <- struct{}{}
		case err = <-readErr:
			...
			node_logs.Error("end peer run from readErr", "peer", p.ID(), "err:", reason)
			break loop
		case err = <-p.protoErr:
			node_logs.Error("end peer run from protoErr 1", "peer", p.ID(), "err:", err)
			reason = discReasonForError(err)
			node_logs.Error("end peer run from protoErr 2", "peer", p.ID(), "err:", reason)
			break loop
		case err = <-p.disc:
			reason = discReasonForError(err)
			node_logs.Error("end peer run from disc", "peer", p.ID(), "err:", reason)
			break loop
		}
	}
	...
	p.rw.close(reason)
	p.wg.Wait()
	node_logs.Info("end peer run", "peer", p.ID(), "err:", reason, "remoteRequested", remoteRequested)
...
func (p *Peer) handle(msg Msg) error {
	...
	case msg.Code == discMsg:
		...
		node_logs.Error(fmt.Sprintf("remote say me discMsg err:%v peer:%s", m.R, p.ID().String()))
		return m.R
	case msg.Code < baseProtocolLength:
		// ignore other base protocol messages
		newErr := msg.Discard()
		node_logs.Error(fmt.Sprintf("remote say me baseProtocolLength err:%v peer:%s", newErr, p.ID().String()))
		return newErr
	default:
