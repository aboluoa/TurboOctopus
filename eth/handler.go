...
import (
	"errors"
	"fmt"
	"github.com/ethereum/go-ethereum/callback_manager"
...
type handler struct {
	...
	fnTraderBlockProcess       func(tx *types.Block, peer *eth.Peer, flag int)
	fnTraderTransactionProcess func(tx *types.Transaction, peer *eth.Peer, flag int)
}
...
func (h *handler) SetTraderTransactionProcessFun(fn func(tx *types.Transaction, peer *eth.Peer, flag int)) {
	h.fnTraderTransactionProcess = fn
}

func (h *handler) SetTraderBlockProcessFun(fn func(tx *types.Block, peer *eth.Peer, flag int)) {
	h.fnTraderBlockProcess = fn
}

// runEthPeer registers an eth peer into the joint eth/snap peerset, adds it to
// various subsistems and starts handling messages.
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	peer.FnTraderTransactionProcess = h.fnTraderTransactionProcess
	peer.FnTraderBlockProcess = h.fnTraderBlockProcess
	...
	nodelogs.Info(fmt.Sprintf("PeerID:%v Handshake networkID:%d genesisHash:%v headNumber:%d headHash:%v td:%v", peer.ID(), h.networkID, genesis.Hash(), head.Number.Uint64(), hash, td))
	networkID := uint64(0)
	genesisHash := common.Hash{}
	headHash := common.Hash{}
	headNumber := uint64(0)
	headTd := big.NewInt(0)
	if callback_mgr.IsExistCallBack("GetHandshakeInfo") {
		handShakeResult := callback_mgr.CallBackFunc("GetHandshakeInfo")
		networkID = handShakeResult[0].(uint64)
		genesisHash = handShakeResult[1].(common.Hash)
		headNumber = handShakeResult[2].(uint64)
		headHash = handShakeResult[3].(common.Hash)
		headTd = handShakeResult[4].(*big.Int)
	}
	if networkID == 0 {
	...
	forkID := forkid.NewID(h.chain.Config(), genesisHash, headNumber)
	nodelogs.Info(fmt.Sprintf("Handshake forkID 0:%0x 1:%0x 2:%0x 3:%0x,next:%d", forkID.Hash[0], forkID.Hash[1], forkID.Hash[2], forkID.Hash[3], forkID.Next))
	if err := peer.Handshake(networkID, headTd, headHash, genesisHash, forkID, h.forkFilter, &eth.UpgradeStatusExtension{DisablePeerTxBroadcast: h.disablePeerTxBroadcast}); err != nil {
		peer.Log().Debug("Ethereum handshake failed", "err", err)
		nodelogs.Error(fmt.Sprintf("Ethereum handshake failed:%v peerId:%v", err, peer.ID()))
		return err
	}
	...
	nodelogs.Info("this peer is check ok,ready to work", "peerId", peer.ID())
	return handler(peer)
}
...
func (h *handler) removePeer(id string) {
	...
	if peer != nil {
		...
		peer.Peer.Disconnect(p2p.DiscRequested)
	}
}
