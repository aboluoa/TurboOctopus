...
func (h *ethHandler) EnqueueBlock(peer string, block *types.Block) error {
	return h.blockFetcher.Enqueue(peer, block)
}
...
func (h *ethHandler) handleBlockAnnounces(peer *eth.Peer, hashes []common.Hash, numbers []uint64) error {
	...
	for i := 0; i < len(unknownHashes); i++ {
		peer.RequestBlocks(unknownHashes[i])
...
func (h *ethHandler) handleBlockBroadcast(peer *eth.Peer, block *types.Block, td *big.Int) error {
	...
	//log.Info("tdddd", "td", td, "blockDiff", block.Difficulty(), "trueTD", trueTD)
