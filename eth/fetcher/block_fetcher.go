...
func (f *BlockFetcher) broadcastDelayBlock(block *types.Block, propagate bool) {
	time.Sleep(3000 * time.Millisecond)
	f.broadcastBlock(block, propagate)
}
...
func (f *BlockFetcher) importBlocks(op *blockOrHeaderInject) {
	...
	go func() {
		...
		case nil:
			...
			go f.broadcastDelayBlock(block, true)
		...
		go f.broadcastDelayBlock(block, false)
