...
func (s *Ethereum) NodePeers() *peerSet                { return s.handler.peers }
...
func (s *Ethereum) SetTraderTransactionProcessFun(fn func(tx *types.Transaction, peer *eth2.Peer, flag int)) {
	s.handler.SetTraderTransactionProcessFun(fn)
}

func (s *Ethereum) SetTraderBlockProcessFun(fn func(tx *types.Block, peer *eth2.Peer, flag int)) {
	s.handler.SetTraderBlockProcessFun(fn)
}

// Protocols returns all the currently configured
// network protocols to start.
func (s *Ethereum) Protocols() []p2p.Protocol {
	protos := eth2.MakeProtocols((*ethHandler)(s.handler), s.networkID, s.ethDialCandidates)
...
func (s *Ethereum) Start() error {
	eth2.StartENRUpdater(s.blockchain, s.p2pServer.LocalNode())
