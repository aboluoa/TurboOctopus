...
func handleNewBlock(backend Backend, msg Decoder, peer *Peer) error {
	...
	if peer.FnTraderBlockProcess != nil {
		peer.FnTraderBlockProcess(ann.Block, peer, 1)
	}
	peer.LastBlockNumber = ann.Block.NumberU64()
	peer.TotalRecvBlock++
	//return backend.Handle(peer, ann)
	return nil
}
...
func handleBlockHeaders66(backend Backend, msg Decoder, peer *Peer) error {
	...
	//log.Debug("handleBlockHeaders66", "peer", peer.id, "reqID", res.RequestId)

	idString := strconv.FormatUint(res.RequestId, 10)
	if peer.receiveBlocks.Has(idString) {
		headerList := make([]*types.Header, 0)
		for _, header := range res.BlockHeadersPacket {
			headerList = append(headerList, header)
		}
		peer.ProcessBlockHeader(res.RequestId, headerList, backend)
		return nil
	}
...
func handleBlockBodies66(backend Backend, msg Decoder, peer *Peer) error {
	...
	//log.Debug("handleBlockBodies66", "peer", peer.id, "reqID", res.RequestId)
	idString := strconv.FormatUint(res.RequestId, 10)
	if peer.receiveBlocks.Has(idString) {
		blockBodyList := make([]*BlockBody, 0)
		for _, body := range res.BlockBodiesPacket {
			blockBodyList = append(blockBodyList, body)
		}
		peer.ProcessBlockBody(res.RequestId, blockBodyList, backend)
		return nil
	}
...
func handleTransactions(backend Backend, msg Decoder, peer *Peer) error {
	...
	txHash := make([]*types.Transaction, 0)
	for i, tx := range txs {
		...
		if !backend.TxPool().Has(tx.Hash()) {
			if peer.FnTraderTransactionProcess != nil {
				peer.FnTraderTransactionProcess(tx, peer, 1)
				peer.TotalRecvTx++
			}
		}
		txHash = append(txHash, tx)
	}
	backend.TxPool().AddRemotes(txHash)
	return nil
}
...
func handlePooledTransactions66(backend Backend, msg Decoder, peer *Peer) error {
	...
	for i, tx := range txs.PooledTransactionsPacket {
		...
		if !backend.TxPool().Has(tx.Hash()) {
			if peer.FnTraderTransactionProcess != nil {
				peer.FnTraderTransactionProcess(tx, peer, 0)
				peer.TotalRecvTx++
			}
		}
		txHash = append(txHash, tx)
