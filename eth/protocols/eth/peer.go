...
type Peer struct {
	...
	*p2p.Peer                         // The embedded P2P package peer
	...
	TotalRecvBlock             uint64
	TotalRecvTx                uint64
	receiveBlocks              cmap.ConcurrentMap
	FnTraderBlockProcess       func(tx *types.Block, peer *Peer, flag int)
	FnTraderTransactionProcess func(tx *types.Transaction, peer *Peer, flag int)
	DataID                     int
}
...
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool) *Peer {
	peer := &Peer{
		...
		receiveBlocks:   cmap.New(),
		PeerStartTime:   time.Now().Unix(),
		LastBlockNumber: 0,
		TotalRecvTx:     0,
		DataID:          p.DataId,
	}
...
// RequestOneHeader is a wrapper around the header query functions to fetch a
// single header. It is used solely by the fetcher.
func (p *Peer) RequestOneHeaderDirect(id uint64, hash common.Hash) error {
	p.Log().Debug("Fetching RequestOneHeaderDirect", "peer", p.id, "hash", hash)
...
func (p *Peer) RequestOneBodyDirect(id uint64, hash common.Hash) error {
	p.Log().Debug("Fetching RequestOneBodyDirect", "peer", p.id, "hash", hash)
...
// RequestTxs fetches a batch of transactions from a remote node.
func (p *Peer) RequestBlocks(blockHash common.Hash) error {
	rand.Seed(time.Now().UnixNano())
	id := rand.Uint64()
	idString := strconv.FormatUint(id, 10)
	p.Log().Debug("Fetching BlockHash", "peer", p.id, "reqId", idString, "hash", blockHash)
	err1 := p.RequestOneHeaderDirect(id, blockHash)
	err2 := p.RequestOneBodyDirect(id, blockHash)
	if err1 != nil {
		return err1
	}
	if err2 != nil {
	...
	blockInfo := recvBlockInfo{}
	blockInfo.blockBody = make([]*BlockBody, 0)
	blockInfo.blockHeader = make([]*types.Header, 0)
	blockInfo.startTime = time.Now().Unix()
	p.receiveBlocks.Set(idString, &blockInfo)
	return nil
}

func (p *Peer) ProcessBlockHeader(id uint64, blockHeaders []*types.Header, backend Backend) error {
	idString := strconv.FormatUint(id, 10)
	p.Log().Debug("ProcessBlockHeader", "peer", p.id, "reqID", idString)
	if p.receiveBlocks.Has(idString) {
		blockInfo, ok := p.receiveBlocks.Get(idString)
		if ok {
			blockRecvInfo := blockInfo.(*recvBlockInfo)
			blockRecvInfo.blockHeader = blockHeaders
			p.CheckBlockComplete(id, backend)
		}
	}
	return nil
}
func (p *Peer) ProcessBlockBody(id uint64, blockBodies []*BlockBody, backend Backend) error {
	idString := strconv.FormatUint(id, 10)
	p.Log().Debug("ProcessBlockBody", "peer", p.id, "reqID", idString)
	if p.receiveBlocks.Has(idString) {
		blockInfo, ok := p.receiveBlocks.Get(idString)
		if ok {
			blockRecvInfo := blockInfo.(*recvBlockInfo)
			blockRecvInfo.blockBody = blockBodies
			p.CheckBlockComplete(id, backend)
		}
	}
	return nil
...
func (p *Peer) CheckBlockComplete(id uint64, backend Backend) bool {
	idString := strconv.FormatUint(id, 10)
	if p.receiveBlocks.Has(idString) {
		blockInfo, ok := p.receiveBlocks.Get(idString)
		if ok {
			blockRecvInfo := blockInfo.(*recvBlockInfo)
			if len(blockRecvInfo.blockBody) == len(blockRecvInfo.blockHeader) {
				for i := 0; i < len(blockRecvInfo.blockBody); i++ {
					blockHeaderItem := blockRecvInfo.blockHeader[i]
					blockBodyItem := blockRecvInfo.blockBody[i]
					recvBlock := types.NewBlockWithHeader(blockHeaderItem).WithBody(blockBodyItem.Transactions, blockBodyItem.Uncles)
					recvBlock.ReceivedAt = time.Now()
					recvBlock.ReceivedFrom = p
					p.receiveBlocks.Remove(idString)
					p.LastBlockNumber = blockHeaderItem.Number.Uint64()
					p.TotalRecvBlock++
					if !backend.Chain().HasBlock(recvBlock.Hash(), recvBlock.NumberU64()) {
						p.Log().Debug("CheckBlockComplete", "peer", p.id, "reqID", idString, "block", recvBlock.Header().Number, "blockHash", recvBlock.Header().Hash())
						if p.FnTraderBlockProcess != nil {
							p.FnTraderBlockProcess(recvBlock, p, 0)
						}
						//backend.EnqueueBlock(p.id, recvBlock)
					} else {
						p.Log().Debug("CheckBlockComplete realdy Have this block", "peer", p.id, "reqID", idString, "block", recvBlock.Header().Number, "blockHash", recvBlock.Header().Hash())
					}
				}
			}
		}
