...
type Backend interface {
	...
	EnqueueBlock(peer string, block *types.Block) error
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	if handler := handlers[msg.Code]; handler != nil {
		if msg.Code == NewPooledTransactionHashesMsg {
			...
			unknowTxHashes := make([]common.Hash, 0)
			for _, hash := range *ann {
				knowTx := backend.TxPool().Get(hash)
				if knowTx == nil {
					unknowTxHashes = append(unknowTxHashes, hash)
					log.Debug("start req tx:", "peer", peer.id, "hash", hash)
					peer.markTransaction(hash)
				} else {
					log.Debug("found ann tx:", "peer", peer.id, "hash", hash)
				}
			}
			if len(unknowTxHashes) > 0 {
				return peer.RequestTxs(unknowTxHashes)
			} else {
				return nil
			}
		}
